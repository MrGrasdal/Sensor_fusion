...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def gate(
        ...
        *,
        ...
        for i in range(M):
            NIS = self.state_filter.NIS(Z[i,:], filter_state, sensor_state=sensor_state)
    ...
    def loglikelihood_ratios(
        ...
        *,
        ...
        for i in range(M):
            ll[i+1] = log_PD + self.state_filter.loglikelihood(Z[i], filter_state, sensor_state=sensor_state)
    ...
    def NEES(
        ...
        error = estimate - true
        return error.T @ np.linalg.inv(covariance) @ error.T
