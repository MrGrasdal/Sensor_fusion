...
class IMM(Generic[MT]):
    ...
    def mix_probabilities(
        ...
        predicted_mode_probabilities, mix_probabilities = (
            discretebayes.discrete_bayes(self.PI, immstate.weights)
        )
    ...
    def mix_states(
        ...
        for i in range(mix_probabilities[:]):
            mix_mean, mix_cov = mixturereduction.gaussian_mixture_moments(
                mix_probabilities[i,:], 
                immstate.components[i].mean, 
                immstate.components[i].cov)
            mixed_states.append(GaussParams(mix_mean, mix_cov))
    ...
    def mode_matched_prediction(
        ...
        modestates_pred = List[MT]

        for i in range(len(mode_states)):
            mode_states_pred.append(self.filters[i].predict(mode_states[i],Ts))
    ...
    def predict(
        ...
        predicted_mode_probability, mixing_probability = self.mix_probabilities(immstatem, Ts)
    ...
    def estimate(self, immstate: MixtureParameters[MT]) -> GaussParams:
        ...
        data_reduced = gaussian_mixture_moments(immstate.w, immstate.mean, immstate.cov)
        estimate =  GaussParams(data_reduced)
        return estimate
