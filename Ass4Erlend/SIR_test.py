...
px = np.array([rng.standard_normal(N) ,rng.standard_normal(N)]).T # TODO
# initial weights
#w = 1/N*np.ones(N) # TODO
w = np.ones(N)
print(w.shape)
...
for k in range(K):
    ...
    for n in range(N):
        ...
        w[n] =w[n]*PF_measurement_distribution.pdf(dz) # TODO, hint: PF_measurement_distribution.pdf
