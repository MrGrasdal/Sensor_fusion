...
class IMM(Generic[MT]):
    ...
    def mix_probabilities(
        ...
        predicted_mode_probabilities, mix_probabilities = (
            discretebayes.discrete_bayes(immstate.weights, self.PI) # TODO hint: discretebayes.discrete_bayes
        )  
    ...
    def mix_states(
        ...
        #mixed_states = gaussian_mixture_moments(mix_probabilities[s,:],immstate.components[s].mean, immstate.components[s].cov)
        
        mixed_states = [] #list(MT)
        for s in range(len(mix_probabilities)):
            mix_mean, mix_cov = gaussian_mixture_moments(mix_probabilities[s,:],immstate.components[s].mean, immstate.components[s].cov)
            mixed_states.append(GaussParams(mix_mean, mix_cov)) # TODO
        return mixed_states
    ...
    def mode_matched_prediction(
        ...
        mode_states_pred = list(MT)
            for s in range(len(mode_states)):
                mode_states_pred.append(self.filters[s].predict(mode_states[s],Ts))
            #mode_states_pred.append(EKF.predict(GaussParams(mode_states[s].mean, mode_states[s].cov)), Ts)
        #modestates_pred = EKF.estimate() # TODO
        return mode_states_pred
    ...
    def mode_matched_update(
        ...
        updated_state = [] #list(MT)
        logLambdas = []
        #S_k = list(MT)
        for s in range(len(self.PI)):
            filter = self.filters[s]
            ekfstate_upd.append(filter.update(z,immstate.components[s],sensor_state=sensor_state)) # = filter.update(z,)
            logLambdas.append(filter.loglikelihood(z, immstate.components[s], Ts,sensor_state=sensor_state))
            #ekfstate_hat = EKF.update(z,GaussParams(immstate.components[s].mean, immstate.components[s].cov))
            #updated_state.append(ekfstate_hat)
            #S_k.append(EKF.innovation_cov(z, ekfstate_hat,sensor_state=sensor_state))
        return updated_state, logLambdas #S_k
    ...
    def update_mode_probabilities(
        ...
        mode_loglikelihood = logsumexp(immstate.weights) # TODO
        # potential intermediate step logjoint =
        # EKF.innovation_cov(z,GaussParams(immstate.components[s].mean, immstate.components[s].cov))
        updated_mode_probabilities =  None # TODO 
    ...
    def update(
        ...
        updated_weights = self.update_mode_probabilities(z,immstate,sensor_state=sensor_state) # TODO
        updated_states = self.update_mode_probabilities(x,immstate,sensor_state=sensor_state) #self.mode_matched_update(z,immstate,sensor_state=sensor_state) # TODO
    ...
    def reduce_mixture(
        ...
        component_conditioned_mode_prob = np.array([c.weights.ravel() for c in immstate_mixture.components])

        # flip conditioning order with Bayes
        mode_prob, mode_conditioned_component_prob = discretebayes.discrete_bayes(immstate_mixture.components.pr, immstate_mixture.components.cond_pr) #None # TODO

        # Hint list_a of lists_b to list_b of lists_a: zip(*immstate_mixture.components)
        mode_states = zip(mode_prob, mode_conditioned_component_prob) #None # TODO: ???
    ...
    def estimate(self, immstate: MixtureParameters[MT]) -> GaussParams:
        ...
        #self.reduce_mixture(immstate)
        data_reduced = gaussian_mixture_moments(immstate.w, immstate.mean, immstate.cov) # TODO
        estimate =  GaussParams(data_reduced) #(EKF.estimate(data_reduced) # TODO
        #self.filters[0].estimate
        return estimate
