...
# Init_modes = ekf.GaussParams(init_mean, init_cov) # init_mean has shape (5,) and init_cov has shape (5, 5) #

# Init_mode_probabiliities = np.array([a, 1-a]) # a in [0,1]
...
# TODO
init_state ={"mean": [0, 0, 2, 0, 0], "cov": np.diag([25, 25, 3, 3, 0.0005]) ** 2 } # {"mean": [None], "cov": np.diag([None]) ** 2}
init_ekfstate = ekf.GaussParams(init_state["mean"], init_state["cov"])
...
filters.append(ekf.EKF(CT, measurement_model)) #(None) # TODO, so you understand what is going on here
...
ANIS = np.array([st['NIS'].mean(axis=0) for st in stats]) #stats_array['NIS'].mean(axis=2) # TODO, hint mean
CINIS = np.array(scipy.stats.chi2.interval(0.9, 2*K)) / K # TODO, hint scipy.stats.chi2.interval
CIANIS =  np.array(scipy.stats.chi2.interval(0.9, 2*K)) / K # TODO, hint [...].inteval
