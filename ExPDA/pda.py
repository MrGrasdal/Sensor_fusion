...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def predict(self, filter_state: ET, Ts: float) -> ET:
        ...
        return state_filter.predict(filter_state, Ts)
    ...
    def gate(
        ...
        *,
        ...
        gated = np.empty(M,dtype=bool) # TODO: some for loop over elements of Z using self.state_filter.gate
        
        for i in range(M):
            NIS = self.state_filter.NIS(Z[i,:], filter_state, sensor_state)
            gated [i] = NIS <= g_squared
    ...
    def association_probabilities(
        ...
        *,
        ...
        # log likelihoods
        lls = self.loglikelihood_ratios(Z, filter_state, sensor_state)

        # probabilities
        beta = np.exp(lls) # TODO
        return beta
    ...
    def conditional_update(
        ...
        *,
        ...
        conditional_update = []
        conditional_update.append(filter_state) # TODO: missed detection  
        # TODO: some loop over Z making a list of updates

        for i in range(Z.shape[0]):
            update = self.state_filter.update(Z[i,:], filter_state, sensor_state)
            conditional_update.append(update)
    ...
    def update(
        ...
        *,
        ...
        filter_state_updated_mixture_components = self.conditional_update(Zg, filter_state, sensor_state) # TODO
    ...
    def step(
        ...
        *,
        ...
        filter_state_updated = self.update(Z, filter_state_predicted, sensor_state) # TODO
